      ...
      <div className="max-w-3xl mx-auto flex flex-col gap-4">
        ...
        <div className={`border rounded-2xl px-5 py-3 flex items-center gap-3
          ...
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0
            ${t.dark ? "bg-amber-500/20 border border-amber-500/40" : "bg-amber-100 border border-amber-300"}`}>
            <span className="text-amber-500 font-black text-xs">AI</span>
          </div>
          <div>
            <p className={`${t.text} text-xs font-bold`}>PaperTrade AI Assistant</p>
            <p className={`${t.muted} text-xs`}>Ask about stocks, strategy, risk management</p>
          </div>
              ...
              <div key={i} className={`flex gap-3 ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                {m.role === "assistant" && (
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mt-0.5
                    ${t.dark ? "bg-amber-500/20 border border-amber-500/40" : "bg-amber-100 border border-amber-200"}`}>
                    <span className="text-amber-500 text-xs font-black">AI</span>
                  </div>
                )}
                <div className={`max-w-lg px-4 py-3 rounded-2xl text-sm leading-relaxed
                  ${m.role === "assistant"
                    ? t.dark
                      ...
                      : "bg-slate-100 text-slate-700 border-l-2 border-amber-500"
                    : t.dark
                      ? "bg-amber-500/20 text-amber-100 border-r-2 border-amber-500"
                      : "bg-amber-500 text-white border-r-2 border-amber-600"}`}>
                  {m.text}
                </div>
                {m.role === "user" && (
                  <div className="w-8 h-8 rounded-xl bg-amber-500 flex items-center justify-center shrink-0 mt-0.5 font-black text-slate-900 text-xs">
                    U
                  </div>
                )}
              </div>
            ...
            {loading && (
              <div className="flex gap-3 items-start">
                ...
                <div className={`px-4 py-3 rounded-2xl flex items-center gap-1.5
                  ${t.dark ? "bg-slate-700/80" : "bg-slate-100"}`}>
                  {[0,1,2].map((i) => (
                    <span key={i} className={`w-2 h-2 rounded-full ${t.dark ? "bg-slate-400" : "bg-slate-400"}`}
        ...
        {/* Suggested questions */}
        <div>
          <p className={`${t.muted} text-xs tracking-widest uppercase font-semibold mb-3`}>
            Suggested Questions
          </p>
